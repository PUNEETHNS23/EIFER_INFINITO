...
import api from '../api';
import { defaultScoreDetail, getSportCategories, getSportMeta, SPORTS } from '../sports/sportsConfig';
...
function createRandomizedDrafts(teams, category, sportId) {
  ...
  const drafts = [];
  const byes = [];
  const isRacketSport = ['badminton', 'table-tennis'].includes(sportId);
  ...
  for (let index = 0; index < shuffledTeams.length; index += 2) {
    ...
    const scoreDetail = {
      ...
      ...(category && ['badminton', 'table-tennis', 'carrom', 'chess', 'kho-kho', 'weight-lifting'].includes(sportId) && { category }),
    };

    // Apply lineup logic only for racket sports
    if (isRacketSport) {
      Object.assign(scoreDetail, buildLineup(team1, category, 'p1', true));
      Object.assign(scoreDetail, buildLineup(team2, category, 'p2', true));
    }
...
export default function AdminAutomaticSchedule() {
  ...
  const isRacketSport = ['badminton', 'table-tennis'].includes(sportId);
  const isValidSport = SPORTS.some((s) => s.id === sportId);
  ...
  const categoryTeams = useMemo(
    () => {
      ...
      return teams.filter((team) => normalizeCategory(team.category) === selectedCategory);
    },
  ...
  const handleSave = async (event) => {
    ...
    try {
      ...
      for (const draftItem of scheduleDrafts) {
        ...
        // Apply category and lineup resolution only for racket sports
        if (isRacketSport) {
          nextDetail.category = selectedCategory;
          Object.assign(nextDetail, resolveLineup(draftItem.score_detail, resolvedTeam1, selectedCategory, 'p1'));
          Object.assign(nextDetail, resolveLineup(draftItem.score_detail, resolvedTeam2, selectedCategory, 'p2'));
      ...
      alert(error.response?.data?.detail || error.message || 'Error creating matches');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="container">
      <div className="page-header">
        ...
        <h1 className="page-title">
          {meta.icon} {meta.name} automatic scheduling
        </h1>
        <p className="hero-subtitle" style={{ marginTop: '0.5rem' }}>
          Build randomized matches for every team in the selected category. Team compositions are auto-generated{isRacketSport ? ' from squads' : ''} and fully editable before saving.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="card">
          <p>At least two teams are required to generate a random schedule{selectedCategory ? ' for this category' : ''}.</p>
        </div>
      ) : (
          ...
          {scheduleDrafts.map((draftItem, index) => {
            ...
            return (
              <div className="card" style={{ marginBottom: '1.25rem' }} key={draftItem.localId}>
                ...
                {isRacketSport && String(draftItem.score_detail.point_limit) === 'Custom' && (
